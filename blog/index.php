...
<div id="blog"><?php
	 ...
	 $pics=scandir("entries/".$e);
	 foreach($pics as $p){
	   if($p!="." && $p!=".."){
             print("<img src = 'entries/".$e."/".$p."'>");
           }
      	 }
	 print("  </div><hr>");
	 print("  <div class='lower'><p class='date'>".$datestring."</p>");
         
	 $query="select name, content, date_format(time, '%e.%c.%Y') as date from comments where post = '". $e . "'";
	 ...
	 print("<input type='hidden' name='post' value='" . $e . "'>");
